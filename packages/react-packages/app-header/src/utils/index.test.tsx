...
describe('AppHeader utils', () => {
  ...
  it('computes bar state for standard layout with app name', () => {
    const state = computeBarState('standard', 'center', 'App', true);
  ...
  it('computes bar state for compact layout without app name', () => {
    const state = computeBarState('compact', 'left', undefined, true);
    expect(state).toMatchObject({
      isStandard: false,
      isCompact: true,
      isNavCenter: false,
  ...
  it('computes bar state for right navigation position', () => {
    const state = computeBarState('compact', 'right', undefined, true);
    expect(state).toMatchObject({
      isStandard: false,
      isCompact: true,
      isNavCenter: false,
  ...
  it('determines when logo should be centered', () => {
    expect(shouldCenterLogo('standard', 'App', 0, 1)).toBe(true);
    expect(shouldCenterLogo('compact', 'App', 0, 1)).toBe(false);
    expect(shouldCenterLogo('standard', undefined, 0, 1)).toBe(false);
  });
  ...
  it('groups adjacent sibling components', () => {
    ...
    const grouped = groupSiblingComponents(
      children,
      ['AppHeader.AppName', 'AppHeader.Logo'],
      false
    );
