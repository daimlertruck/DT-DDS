...
describe('<AppHeader /> component', () => {
  ...
  describe('Component grouping', () => {
    it('does not group AppName and Logo on mobile compact layout', () => {
      mockUseMedia.mockReturnValue(true);

      const { container } = render(
        <ProvidedAppHeader type='compact'>
          <AppHeader.AppName data-testid='app-name-mobile' name='Mobile App' />
          <AppHeader.Logo data-testid='logo-mobile'>
            <img alt='Mobile Logo' src='/mobile-logo.svg' />
          </AppHeader.Logo>
        </ProvidedAppHeader>
      );

      const header = container.querySelector('header');
      const appName = screen.getByTestId('app-name-mobile');
    ...
    it('groups AppName and Logo on desktop compact when siblings', () => {
      mockUseMedia.mockReturnValue(false);
