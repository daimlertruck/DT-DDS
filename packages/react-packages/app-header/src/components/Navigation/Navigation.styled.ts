...
const shouldUseAbsoluteCenter = (state: BarState): boolean => {
...
const getNavigationCenteringStyles = (state: BarState): string => {
...
const calculatePaddingLeft = (
  isStandardAndHasAppName: boolean,
  isNavLeft: boolean,
  spacing: string
): number | string => {
  if (!isStandardAndHasAppName) {
    return 0;
  }
  if (!isNavLeft) {
    return 0;
