...
- **`hide`**: Hides the component when the media query matches (e.g., `hide="(min-width: 768px)"` hides on desktop)
- **`show`**: Shows the component only when the media query matches (e.g., `show="(max-width: 767px)"` shows only on mobile)
